...
## Create ECS Cluster infra
    ...
    aws cloudformation deploy --template-file 4-ecs-cluster.yml --stack-name bala-ecs-cluster-infra --capabilities CAPABILITY_NAMED_IAM 

## Create ECS Service and Task infra

    aws cloudformation deploy --template-file 5-ecs-service-task.yml  --stack-name bala-ecs-service-task-infra --capabilities CAPABILITY_NAMED_IAM 

### Invoke calculator api using ALB

    curl  "<ALB-DNS-Name>/actuator/health/"

    curl  "http://Bala-MicroserviceALB-966177091.us-east-1.elb.amazonaws.com/actuator/health/"
